...
class Map extends Base {
    constructor(id, isBattleMap = false, minimal = false, heroOrientation = null) {
        ...
        this.landObjectsSpatialHash = new globalThis.Map();
        this.landObjectsSpatialHashDirty = true;
    ...
    updateCameraHiding(targets) {
        ...
        for (const target of targets) {
            ...
            const distance = Math.max(minimumDistance, this.camera.targetPosition.clone().sub(point).dot(cameraDirection) - margin);
    ...
    update() {
        ...
        if (Game.current !== null) {
            ...
            if (!this.isBattleMap && Game.current.caterpillarFollowers.length > 0) {
                ...
                const offset = Data.Systems.caterpillarOffset.getValue() / Data.Systems.SQUARE_SIZE;
        ...
        if (Game.current !== null && Data.Systems.moveCameraOnBlockView.getValue()) {
            ...
            const targets = [];
            const horizontalDirection = new THREE.Vector3(-Math.sin(this.camera.horizontalAngle), 0, Math.cos(this.camera.horizontalAngle));
            for (const height of [0.1, 0.5, 0.9]) {
                for (const width of [-0.4, 0, 0.4]) {
                    targets.push(hero.position
                        .clone()
    ...
    async buildCaterpillarFollowers(resetTrail = true, restoreStates) {
        ...
        const spawnStep = Data.Systems.caterpillarOffset.getValue() / Data.Systems.SQUARE_SIZE + 1;
        if (resetTrail) {

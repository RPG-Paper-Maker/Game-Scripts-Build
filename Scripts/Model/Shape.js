...
export class Shape extends Base {
     ...
     */
    static async parseGLTF(buffer, squareSize) {
        ...
        gltf.scene.traverse((child) => {
            ...
            for (let i = 0; i < posAttr.count; i++) {
                const vertex = new THREE.Vector3(posAttr.getX(i) * squareSize, posAttr.getY(i) * squareSize, posAttr.getZ(i) * squareSize);
    ...
    async checkBase64() {
        if (!Platform.IS_DESKTOP && !this.isBR && Platform.WEB_DEV) {
            this.base64 = await Platform.loadFile(`${Platform.ROOT_DIRECTORY}${Shape.getLocalFolder(this.kind)}/${this.name}`);
        }
    }
    /**

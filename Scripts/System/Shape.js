    ...
    static parse(text) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.length === 0 || line.charAt(0) === '#') {
                ...
                temp3D = new THREE.Vector3(parseFloat(result[1]) * Datas.Systems.SQUARE_SIZE, parseFloat(result[2]) * Datas.Systems.SQUARE_SIZE, parseFloat(result[3]) * Datas.Systems.SQUARE_SIZE);
                v.push(temp3D);
                if (firstVertex) {
        ...
        object.center = new THREE.Vector3((maxVertex.x - minVertex.x) / 2 + minVertex.x, (maxVertex.y - minVertex.y) / 2 + minVertex.y, (maxVertex.z - minVertex.z) / 2 + minVertex.z);

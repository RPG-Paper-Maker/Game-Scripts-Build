...
*/
import { CHARACTER_KIND, Utils } from '../Common/index.js';
        ...
        if (player) {
            if (this.battlerID) {
                if (this.permanent) {
                    ...
                    player.temporaryBattlerID = this.battlerID.getValue();
                }
            }
            if (this.facesetID) {
                if (this.permanent) {
                    ...
                    player.temporaryFacesetID = this.facesetID.getValue();
            ...
            if (this.pose && battler) {
                battler.setDefaultStep(this.pose.getValue());
            }
            if (this.permanent && Scene.Map.current.isBattleMap && map.windowExperienceProgression) {
                map.windowExperienceProgression.content.updateGraphicCharacters();
            }
        }
        return 1;
    }
